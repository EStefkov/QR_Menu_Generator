...
public class MenuController {
    ...
    @PostMapping("/{menuId}/default-product-image")
    public ResponseEntity<MenuDTO> uploadDefaultProductImage(
            @PathVariable Long menuId,
            @RequestParam("defaultProductImage") MultipartFile file) {
        try {
            MenuDTO updatedMenu = menuService.uploadDefaultProductImage(menuId, file);
            return ResponseEntity.ok(updatedMenu);
        } catch (Exception e) {
            throw new RuntimeException("Failed to upload default product image: " + e.getMessage());
        }
    }
}
